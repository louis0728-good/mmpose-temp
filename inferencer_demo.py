...
print(f"[DEBUG] 現在使用的torch裝置: {torch.cuda.current_device() if torch.cuda.is_available() else 'cpu'}")
...
def parse_args():
    ...
    parser.add_argument(
        '--tracking-thr', type=float, default=0.7, help='Tracking threshold')
    parser.add_argument(
        '--use-oks-tracking',
        action='store_true',
